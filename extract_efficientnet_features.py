...
def resolve_local_image_path(img_url: str, image_dir: Path) -> Path | None:
    """
    Пример:
    /daily-rebuses/1_v1.jpg -> images/daily-rebuses/1_v1.jpg.

    Если в датасете указан файл с суффиксом _v1, а в папке лежит базовое
    имя без суффикса, пробуем fallback: 1_v1.jpg -> 1.jpg.
    """
    if pd.isna(img_url):
        return None
